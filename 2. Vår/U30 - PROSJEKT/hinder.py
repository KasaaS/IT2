...
class Hinder(Figur):
    def __init__(self, x: int, y: int, x_fart: int = None, y_fart: int = None) -> None:
        super().__init__(x, y, 50, 50, "orange")

        # Hvis x_fart eller y_fart ikke er satt, velg en tilfeldig retning
        if x_fart is None:
            x_fart = 2 if random.randint(0,1) == 1 else -2
        if y_fart is None:
            y_fart = 2 if random.randint(0,1) == 1 else -2
        self.x_fart = x_fart
        self.y_fart = y_fart

    # Funksjon som flytter
    def flytt(self) -> None:
        self.rect.x += self.x_fart
        self.rect.y += self.y_fart

    # Funksjoner som snur retning
    def snu_x(self) -> None:
        self.x_fart *= -1

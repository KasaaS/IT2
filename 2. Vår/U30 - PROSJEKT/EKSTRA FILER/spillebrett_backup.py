...
## Oppdaterer her
    ...
    def aktiver_boosters(self):
        ...
        if self.tid_mellom_booster > 240:
        ...
        for booster in self.boostere:
            for ball in self.baller:
                if ball.rect.colliderect(booster.rect):
                    ...
                    # Sjekker hva tilfeldig boost returnerer, og endrer ballens egenskaper basert på det. Boost er permanent.
                    if booster.tilfedlig_boost() == "MAXFART":
                        ball.max_fart()
                    if booster.tilfedlig_boost() == "MINFART":
                        ball.min_fart()
